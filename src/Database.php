...
class Database implements \IteratorAggregate, \Countable {
    ...
    /**
     * Get rows from table
     * @uses jsonDB\Data::get() to get data from file
     * @return array
     */
    ...
    protected function getRowKey($id)
    {
        ...
        throw new dbException('No data found with ID: ' . $id);
    }
     ...
     * @uses jsonDB\Validate::isNumeric() to check if type of field is numeric
     */
    protected function setFields()
    {
        ...
        {
            if (Validate::isNumeric($type) AND $field != 'id')
            {
                $this->set->{$field} = 0;
            }
            else
     ...
     * @uses jsonDB\Validate::field() to check that field exist
     * @uses jsonDB\Validate::type() to check that field type is correct
    ...
    public function __get($name)
    {
        ...
        throw new dbException('There is no data');
    }
     ...
     *
     * @uses jsonDB\Data::arrToLower() to lower case keys and values of array
     * @uses jsonDB\Data::exists() to check if data file exists
     * @uses jsonDB\Config::exists() to check if config file exists
     * @uses jsonDB\Validate::types() to check if type of fields are correct
     * @uses jsonDB\Data::put() to save data file
     * @uses jsonDB\Config::put() to save config file
     ...
     * @uses jsonDB\Data::remove() to remove data file
     * @uses jsonDB\Config::remove() to remove config file
     * @param string $name Table name
