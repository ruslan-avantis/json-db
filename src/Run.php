<?php
	...
	class Db {
		...
		*/
		...
		private $api				= true; //	Активация API. true|false Если установить false база будет работать как основное хранилище
								...
								if ($unit["action"] == 'update' || $unit["action"] == 'create') {
									...
									if ($unitCount >= 1) {
										foreach($unit["relations"] as $rel_key => $rel_value){
											$has = $rel_value["type"];
											Relation::table($unit["table"])->$has($rel_key)->localKey($rel_value["keys"]["local"])->foreignKey($rel_value["keys"]["foreign"])->setRelation();
										}
									}
								}
